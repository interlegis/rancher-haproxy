...
for portcfg in ${PORTS[*]}; do
  ...
  if [ ! -f "/opt/tools/confd/etc/conf.d/frontend_${port}.cfg.toml" ]; then
    ...
    if [ "$cfg" == "ssl" ]; then 
      sed -e "s/%port%/$port/g" -e "s/%porthigh%/$porthigh/g" /opt/tools/confd/etc.tmpl/templates/10-frontend_%port%-ssl.cfg.tmpl > /opt/tools/confd/etc/templates/10-frontend_${port}.cfg.tmpl
    else 
      sed -e "s/%port%/$port/g" -e "s/%porthigh%/$porthigh/g" /opt/tools/confd/etc.tmpl/templates/10-frontend_%port%.cfg.tmpl > /opt/tools/confd/etc/templates/10-frontend_${port}.cfg.tmpl
    fi
  else
